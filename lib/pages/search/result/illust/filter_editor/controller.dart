...
class SearchIllustFilterEditorController extends GetxController {
  final VoidCallback onFilterChanged;

  SearchIllustFilterEditorController(this.onFilterChanged);

  final editFilterPanelController = ExpandableController();

  final SearchIllustFilter filter = SearchIllustFilter.create();
  ...
  SearchIllustTarget get target => filter.target;
  ...
  void searchIllustTargetOnChanged(SearchIllustTarget? value) {
    if (null != value) {
      filter.target = value;
      update();
      onFilterChanged();
