...
class MyPixivNewContent extends StatefulWidget {
  final bool expandTypeSelector;

  const MyPixivNewContent({Key? key, required this.expandTypeSelector}) : super(key: key);

  @override
  State<MyPixivNewContent> createState() => _MyPixivNewContentState();
}

class _MyPixivNewContentState extends State<MyPixivNewContent> {
  @override
  void didUpdateWidget(covariant MyPixivNewContent oldWidget) {
    if (widget.expandTypeSelector != oldWidget.expandTypeSelector) {
      Get.find<MyPixivNewController>().expandableController.expanded = widget.expandTypeSelector;
    }
  ...
  Widget build(BuildContext context) {
    Get.put(MyPixivNewController());
    return GetBuilder<MyPixivNewController>(
