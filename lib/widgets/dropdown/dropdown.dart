...
///
/// Used by [DropdownButtonWidget.selectedItemBuilder].
typedef DropdownButtonWidgetBuilder = List<Widget> Function(BuildContext context);
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  void _handleOnTap() {
    final DropdownMenuItemWidget<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
...
/// An item in a menu created by a [DropdownButtonWidget].
///
...
class DropdownMenuItemWidget<T> extends _DropdownMenuItemContainer {
  ...
  ///
  /// Eventually returned in a call to [DropdownButtonWidget.onChanged].
  final T? value;
...
/// An inherited widget that causes any descendant [DropdownButtonWidget]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [DropdownButtonWidget] widgets placed within material data tables, as
/// required by the material design specification.
class DropdownButtonWidgetHideUnderline extends InheritedWidget {
  /// Creates a [DropdownButtonWidgetHideUnderline]. A non-null [child] must
  /// be given.
  const DropdownButtonWidgetHideUnderline({
    Key? key,
    required Widget child,
  }) : super(key: key, child: child);

  /// Returns whether the underline of [DropdownButtonWidget] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<DropdownButtonWidgetHideUnderline>() != null;
  }

  @override
  bool updateShouldNotify(DropdownButtonWidgetHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [DropdownMenuItemWidget] in [items] must be
...
/// See also:
///
///  * [DropdownButtonWidgetFormField], which integrates with the [Form] widget.
///  * [DropdownMenuItemWidget], the class used to represent the [items].
///  * [DropdownButtonWidgetHideUnderline], which prevents its descendant dropdown buttons
...
class DropdownButtonWidget<T> extends StatefulWidget {
  ...
  /// must be equal to one of the [DropdownMenuItemWidget] values. If [items] or
  ...
  DropdownButtonWidget._formField({
    Key? key,
    required this.items,
    this.selectedItemBuilder,
    this.value,
  ...
  /// The value of the currently selected [DropdownMenuItemWidget].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [DropdownButtonWidget.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [DropdownButtonWidget.disabledHint] widget if it is non-null.
  /// If [DropdownButtonWidget.disabledHint] is also null but [DropdownButtonWidget.hint] is
  /// non-null, [DropdownButtonWidget.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [DropdownMenuItemWidget]s in [items].
  ///
  /// When a [DropdownMenuItemWidget] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropdownMenuItemWidget] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [DropdownMenuItemWidget].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [DropdownMenuItemWidget] from [items]
  /// that matches [value] will be displayed.
  final DropdownButtonWidgetBuilder? selectedItemBuilder;
...
class _DropdownButtonWidgetState<T> extends State<DropdownButtonWidget<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.items!.where((DropdownMenuItemWidget<T> item) => item.value == widget.value).length == 1);
      ...
      innerItemsWidget = IndexedStack(
        ...
        children: widget.isDense
            ...
            : items.map((Widget item) {
                return widget.itemHeight != null ? SizedBox(height: widget.itemHeight, child: item) : widget;
              }).toList(),
      );
    }
...
/// A [FormField] that contains a [DropdownButtonWidget].
///
/// This is a convenience widget that wraps a [DropdownButtonWidget] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [DropdownButtonWidget], which is the underlying text field without the [Form]
///    integration.
class DropdownButtonWidgetFormField<T> extends FormField<T> {
  /// Creates a [DropdownButtonWidget] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  DropdownButtonWidgetFormField({
    Key? key,
    required List<DropdownMenuItemWidget<T>>? items,
    DropdownButtonWidgetBuilder? selectedItemBuilder,
    T? value,
    Widget? hint,
    Widget? disabledHint,
    required this.onChanged,
    ...
    // DropdownButton.
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            ...
            return DropdownButtonWidgetHideUnderline(
              child: DropdownButtonWidget<T>._formField(
                items: items,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  ...
  void didChange(T? value) {
    ...
    final DropdownButtonWidgetFormField<T> dropdownButtonFormField = widget as DropdownButtonWidgetFormField<T>;
    assert(dropdownButtonFormField.onChanged != null);
    dropdownButtonFormField.onChanged!(value);
  }

  @override
  void didUpdateWidget(DropdownButtonWidgetFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
